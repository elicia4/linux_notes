...
:pu=range(start,end,jump) = will output start, start+jump, start+2*jump ... as
long as start+n*jump <= end. So, to output odd numbers that are fewer than or
equal 25:
...
:registers = shows us what is in all the current registers
	vim has many clipboards, they are called registers, you have a register for
	every (?) alphanumeric key and some others
