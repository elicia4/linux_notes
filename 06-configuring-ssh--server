...
The 'sshd' binary represents the server. sshd means ssh daemon. Check the status
of it with:

    systemctl status sshd

You can use all systemctl commands to manage the service. 'restart', 'stop', do 
whatever you want. When you stop the sshd service it's not going to terminate 
any connections that are existing. But if you disconnect, you won't be able to 
reconnect since the sshd service has to be running in order to establish new 
connections. Ubuntu and Debian call the binary 'ssh', not 'sshd'. To start and
enable the service at the same time.

    systemctl enable --now sshd 

Take a look at the /etc/ssh directory. There are keys there stored for the 
fingerprints. When you copy images, ssh keys are copied along with them, and you
don't want all your servers to have the same ssh keys. Make sure to regenrate 
them. 
...
already set up, you wanna make sure that you log in with your ssh key BEFORE you
set this to 'no'.

After configuration, restart the service and make sure the restart didn't fail:
